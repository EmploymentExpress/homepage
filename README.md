...
- **Automation-ready Alert Monitor** — the monitor and workflow template are configured to check official pages/RSS feeds once every six hours for recruitment, admission, answer-key, result, corrigendum and addendum notices; linked HTML/PDF details are extracted into the generated feed. Per-source reachability is tracked in `data/seen-notices.json` (`sourceHealth`: consecutive failures + last error are committed visibly), unreachable sources can fall back to read-only mirrors (`"proxyFallback": true`, currently DESGPC), and the first scan of a new source publishes real current notices rather than portal/navigation links
...
- **Last Date Reminders** — auto-populated 2 days (48 hours) before each **non-extended** job's `lastDate`, with a live per-second countdown timer; jobs leave the list automatically once their deadline passes
- **Last-date extension handling** — when the monitor finds an official corrigendum extending a deadline, the affected job is kept in its relevant recruitment/admission column and the master table, marked **Last Date Extended** with the old date struck through → new date and a link to the notice. Extended jobs are intentionally excluded from the separate Last Date Reminders column, and the **Last Date Extended** tag appears strictly and only on that job details whose last date extended (cards, table, and the Job Details modal).
- **Job-details headlines** — generated as `<Department name> <Post name(s)> Recruitment | Apply Online/Offline` (e.g. `PGIMER Chandigarh DEO, MTS, Pharmacist Recruitment | Apply Online`): the post name is shown for a single post, all 2–4 posts are listed when the notice names that many, `Various Post` is used beyond 4, and the department appears alone when no post is named. Non-recruitment notices are headed `<Department name> <Notice type>` (e.g. `SBI Admit Card`, `PGIMER Chandigarh Result`), and interview call letters render in the Admit Card column.
...
├── tests/test_job_posting_schema.py         # Google Search Console JobPosting schema guard tests
...
1. **Frontend (`index.html`)** — `renderLastDateReminders()` scans `jobDatabase` for **non-extended** jobs whose `lastDate` (`DD-MM-YYYY`, optionally `DD-MM-YYYY HH:MM`) falls within the next 48 hours and renders each one with a live countdown that ticks every second. Jobs with `lastDateExtended: true` are not placed in Last Date Reminders; they stay in their relevant recruitment/admission column and master table row. The green **Last Date Extended** tag is displayed strictly and only on that job details whose last date extended (including the job card, the master table row, and the Job Details modal popup), showing the original date ~~struck through~~, the new date, and a “View notice” link to the official corrigendum.
