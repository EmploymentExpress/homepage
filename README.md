...
- **📄 Offline application forms** — offline-apply vacancies carry a **Download Offline Application Form** link sourced from an external offline-form portal. The monitor reads each vacancy page on the portal and links the **direct application-form PDF** (e.g. a `wp-content/uploads/...-Application-Form-...pdf` file) instead of the vacancy page; the official-notice link uses the portal-hosted **notification PDF** when the official website has no copy of it. The portal's URL and branding are masked: the visible link points to `redirect.html?f=<token>`, which resolves the real URL from `data/offline-redirects.json` (the external URL never appears on the page or in any visible link)
